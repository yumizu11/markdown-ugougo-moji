...
Obsidian のノートに書いた文字を、90 年代の TV 番組「ウゴウゴルーガ」風に**うにょうにょ揺れる**グラフィックとして表示するプラグインです。
...
これが読み取りビューで、輪郭がカクカクと沸き立つ文字になります。

同じ記法を **Marp スライド**でも使えます（[Marp スライドで使う](#marp-スライドで使う)）。
